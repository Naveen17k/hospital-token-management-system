...
export default function DoctorPanel() {
  ...
  const [highlightedRowIndex, setHighlightedRowIndex] = useState(null); // State to store highlighted row index

  useEffect(() => {
    const savedTokensData = JSON.parse(localStorage.getItem("tokensData"));
    const savedCurrentTokenIndex = JSON.parse(
      localStorage.getItem("currentTokenIndex")
    );
    const savedCurrentTokenNumber = JSON.parse(
      localStorage.getItem("currentTokenNumber")
    );

    const savedOrderedTokens = JSON.parse(
      localStorage.getItem("orderedTokens")
    );
    const savedHighlightedRowIndex = JSON.parse(
      localStorage.getItem("highlightedRowIndex")
    ); // Load highlighted row index

    if (savedTokensData) setTokensData(savedTokensData);
    if (savedCurrentTokenIndex) setCurrentTokenIndex(savedCurrentTokenIndex);
    if (savedCurrentTokenNumber) setCurrentTokenNumber(savedCurrentTokenNumber);
    if (savedOrderedTokens) setOrderedTokens(savedOrderedTokens);
    if (savedHighlightedRowIndex !== null)
      setHighlightedRowIndex(savedHighlightedRowIndex); // Set highlighted row index
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("orderedTokens", JSON.stringify(orderedTokens));
    localStorage.setItem(
      "highlightedRowIndex",
      JSON.stringify(highlightedRowIndex)
    ); // Save highlighted row index
  }, [
    tokensData,
    currentTokenIndex,
    currentTokenNumber,
  ...
  const handleShutdownConfirmation= async () => {
    ...
    // Close confirmation dialog after confirmation
    setShowConfirmation(false);
  };

  const handleShutdownCancel = () => {
    // Close confirmation dialog on cancellation
    setShowConfirmation(false);
  };
  return (
            ...
            <div className="overflow-x-auto max-w-max h-[451px]  scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
              <table className="min-w-full">
                ...
                <tbody className="bg-[#0C89E7] bg-opacity-20 divide-y-2 divide-white">
                  ...
                  {[...orderedTokens, ...callLaterTokens].map(
                    (token, index) => (
                      <tr key={index}>
                        <td className="py-4 pl-4 pr-3 text-center text-sm font-medium text-black sm:pl-3">
                          {token.tokenNumber}
